...
the membership of `Node` must remain _open_;
you do _not_ want to use a sum type with a closed list of concrete members here,
or it will cause other roadblocks later that _will_ force a redesign).
...
With all seven (?! indeed, _seven_) of these different stories,
...
interface, rather than _in_ the `Node` interface, it becomes much more
